...
class Exp_MSN(Exp_Basic):
    ...
    def train(self, setting):
        ...
        print(f"global period weight: {self.args.period_weight}")
        train_data, train_loader = self._get_data(flag='train')
        ...
        if self.args.cotrain:   
            stat_path = os.path.join(self.args.stat_checkpoints, setting)
        ...
        for epoch in range(self.args.train_epochs + self.station_pretrain_epoch):
            ...
            if epoch + 1 <= self.station_pretrain_epoch:
                ...
                if self.args.cotrain:
                    torch.save(self.statistics_pred.state_dict(), stat_path + '/' + 'checkpoint.pth')
